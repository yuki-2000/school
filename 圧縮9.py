...
#上流に対する相対速度
Ms1 = get_Ms(gamma1, p2_p1)
#実験室座標のm\s
vs1 = MsR2vs(Ms1, u1, a1)
...
#上流に対する相対速度
Ms2 = get_Msr(gamma1, p5_p2)
#実験室座標のm\s
vs2 = MsL2vs(Ms2, u2, a2)
...
for i in range(1,n+1):  
    ...
    #each_expansion_fan.plot(x="x", xlim=[-L4, L1], ylim=[0,0.01])
    #交点ににおいてmerge時に順番が飛び、うまく書けない
    #inner_expansion_fan = pd.merge(inner_expansion_fan,each_expansion_fan, on="x", how='outer')
    #下につなげるだけでうまくいく
    inner_expansion_fan = inner_expansion_fan.append(each_expansion_fan)
    
    

#inner_expansion_fan= inner_expansion_fan.sort_values("x")
#inner_expansion_fan.plot(x="x", xlim=[-L4, L1], ylim=[0,0.02])
#inner_expansion_fan.plot.scatter(x="x", y="ex1", xlim=[-L4, L1], ylim=[0,0.01])
#inner_expansion_fan.plot(x="x", y="ex3", xlim=[-L4, L1], ylim=[0,0.02])
...
ax3 = inner_expansion_fan.plot(x="x", legend = False, ax =ax2)
...
if acimp3 > acimp2:
    ...
    print("有効試験時間は" + str(valid_test_t) + "s")


    df_uniue_line = pd.DataFrame({
        "x" : x,
        "sw4":sw4,
    })    

    img = df_uniue_line.plot(x="x",xlim=[-L4, L1], ylim=[0,0.009], figsize=(10, 10), legend = False, ax =ax3, xlabel="x [m]", ylabel="t [s]", title="shock tube n={}".format(n),lw=5).get_figure()
    #img.savefig("圧縮10img_{}".format(n))
    ...
    valid_test_t = expR2_line[-1] - swi_line[-1]
    print("有効試験時間は" + str(valid_test_t) + "s")
    ...
    img = df_uniue_line.plot(x="x",xlim=[-L4, L1], ylim=[0,0.014], figsize=(10, 10), legend = True, ax =ax3, xlabel="x [m]", ylabel="t [s]", title="shock tube").get_figure()
    #img.savefig("img9")
...
state= pd.DataFrame(
    data = np.array([[x0,p4,ro4,u4],
                    ...
                    [x5,p1,ro1,u1]]),
    index = [1,2,3,4,5,6,7,8],
    columns = ["x","圧力[Pa]", "密度[kg/m^3]", "流速[m/s]"]
    )



state.plot(x="x", y= "密度[kg/m^3]")

img = state.plot(x="x",y= "圧力[Pa]", xlim=[-L4, L1], ylim=[0,4.1*10**6], figsize=(10, 10), legend = False,  xlabel="x [m]", ylabel="pressure [Pa]", title="shock tube pressure").get_figure()
img = state.plot(x="x",y= "密度[kg/m^3]", xlim=[-L4, L1], ylim=[0,25], figsize=(10, 10), legend = False,  xlabel="x [m]", ylabel="density [kg/m^3]", title="shock tube density").get_figure()
img = state.plot(x="x",y= "流速[m/s]", xlim=[-L4, L1], ylim=[0,550], figsize=(10, 10), legend = False,  xlabel="x [m]", ylabel="speed [m/s]", title="shock tube speed").get_figure()
#img.savefig("img_{}".format(n))